...
class Chart extends PureComponent {
        ...
        for (let i = 0; i < t.length; t++) {
            tabs.push(
                <TouchableHighlight key={i}
                    style={[...t[i].style, { backgroundColor: this.currentTypeSelect == i ? StreamColor : TitleColor }]}
                    onPress={() => this._onSelectType(t[i], i)}
                    underlayColor={'#28282844'}
                >
                    <View>
                        <Text style={[...t[i].textStyle, { color: this.currentTypeSelect == i ? StreamColor : TitleColor }]} >{t[i].label}</Text>
                    </View>
                </TouchableHighlight>
            )
        }
    ...
    rightTab = () => {
        ...
        for (let i = 0; i < t.length; t++) {
            tabs.push(
                <TouchableHighlight key={i}
                    style={[...t[i].style, { backgroundColor: this.currentTimeSelect == t[i].label ? StreamColor : TitleColor }]}
                    onPress={() => this._onSelectTime(t[i])}
                    underlayColor={'#28282844'}
                >
                    <View>
                        <Text style={[...t[i].textStyle, { color: this.currentTimeSelect == t[i].label ? StreamColor : TitleColor }]} >{t[i].label}</Text>
                    </View>
                </TouchableHighlight>
            )
        }
