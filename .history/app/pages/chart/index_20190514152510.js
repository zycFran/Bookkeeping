...
class Chart extends PureComponent {
    ...
    text() {
        ...
        for (let i = 0; i < count; i++) {
            if (this.state.currentTimeSelect == '周') {
                ...
                var style = [BodyStyles.bottomText, { left: i * padding, width: 35 }];
                arr.push(
                    <Text key={i} style={style}>{'周' + week[i]}</Text>
                )
            } else if (this.state.currentTimeSelect == '月') {
                var padding = (ScreenWidth - 185 * 2) / (count - 1);
                var style = [BodyStyles.bottomText, { left: 6 + i * padding, width: 12 }];
                ...
                var style = [BodyStyles.bottomText, { left: 5 + i * padding, width: 20 }];
                if (i == 0 || (i + 1) % 3 == 0) {
                    arr.push(
    ...
    polyline() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (data != undefined) {
                value = 90 - 90.0 / this.state.data.max * data.value + 5;
                console.log(this.state.data.max);
            }
