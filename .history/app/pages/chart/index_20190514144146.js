        ...
        for (let i = 0; i < t.length; t++) {
            tabs.push(
                <TouchableHighlight key={i}
                    style={[...t[i].style, { backgroundColor: this.state.currentTypeSelect == i ? TitleColor : TitleColor }]}
                    onPress={() => this._onSelectType(t[i], i)}
                    underlayColor={'#28282844'}
                >
                    <View key={i}>
                        <Text style={[...t[i].textStyle, { color: this.state.currentTypeSelect == i ? StreamColor : StreamColor }]} >支出</Text>
                    </View>
                </TouchableHighlight>
            )
        }
